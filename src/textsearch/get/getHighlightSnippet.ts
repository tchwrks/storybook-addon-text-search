 ...
 */
export function getResultHighlightSnippet({ content, query, radius = 50, highlightColor = "#ff0" }: GetHighlightSnippetOptions): string {
    ...
    return snippet.replace(new RegExp(`(${query})`, "gi"), `<mark style="background-color: ${highlightColor}">$1</mark>`);
};